...
ドメインサービス関連インターフェース（`IDomainService<TReq>` 等）、
`ILengthDefinedSingleValueObject`、および利用者向け例外クラス 6 種の削除。
`IValueObject` / `IEntity<TIdentifier>` への `Validate()` の復活。
**破壊的変更を含みます。**

### Breaking Changes

#### `IValueObject` / `IEntity<TIdentifier>`

- `ILengthDefinedSingleValueObject` を削除しました。長さの上下限を公開するためだけの
  マーカーインターフェースでしたが、他の値オブジェクト関連の型に比べて汎用性が低く、
  ジェネリック制約以外の用途がありませんでした。上下限を公開したい値オブジェクトは、
  これまでどおり `static int MaxLength` / `MinLength` を**素の static メンバーとして**
  宣言してください（interface を介さなくなるだけで、公開の仕方自体は変わりません）。

#### ドメインサービス

- **`IDomainService<TReq>` / `IDomainService<TReq, TRes>` / `IDomainServiceDTO` を削除しました。**
  集約サービスやユースケースと違い、ドメインサービスは扱う対象・引数・戻り値の形が
  プロジェクトごとに柔軟で、共通の親インターフェースを立てても「メソッド名と Req/Res の形を
  強制するだけ」の効果しかなく、実際に使う場面がほとんどありませんでした。
  ドメインサービスの口はこれからプロジェクト自身で宣言してください。
  「専用の口を立て、リクエストをその中に `Req` としてネストする」という規約自体は変わりません
  （`IDomainService<TReq>` を継承する代わりに `ExecuteAsync` を自分で 1 つ宣言するだけです）。
...
- **`DomainInvalidOperationException` / `ObjectAlreadyExistException` / `ObjectNotFoundException` /
  `ValueObjectInvalidException` / `ValueObjectLengthException` / `ValueObjectNullException` を
  削除しました。** ライブラリ自身が投げる `NestedTransactionException` /
  `TransactionSessionNotFoundException`（と基底の `TADAException`）以外の例外は、
  中途半端なヘルパーを提供するより、各プロジェクトが自分のドメインの語彙で定義したほうが
  健全と判断しました。これらを使っていたコードは、プロジェクト側で相当する例外型を
  自前で定義して置き換えてください
  → [samples/CSStack.TADA.Sample/Domain/UserExceptions.cs](samples/CSStack.TADA.Sample/Domain/UserExceptions.cs)

### Added

- **`IValueObject.Validate()` / `IEntity<TIdentifier>.Validate()` を追加しました。**
  v2.0.1 で一度削除した `Validate` メンバーの復活です（当時の削除理由 →
  [docs/migration.md](docs/migration.md)）。運用してみると、「外から呼べる検証がある
  = 未検証の値オブジェクトが存在しうる」という当時の懸念よりも、`Reconstruct` で
  古いルールのデータを復元した後に不変条件を再チェックしたい場面のほうが実用上多いと
  判断しました。構築時の検証は引き続き `Create` の仕事で、`Validate()` はそれを置き換える
  ものではありません。戻り値は `void` 固定で、破っていれば例外を投げる運用です
  （投げる例外の型はライブラリが指定しません。上記の例外クラスの削除を参照）。
  **このライブラリの SemVer 方針では、既定実装のないメンバーのインターフェース追加は
  MAJOR 相当です。** `IValueObject` / `IEntity<TIdentifier>` を直接実装している型は
  すべて `Validate()` の実装が必要です。

## [3.0.1] - 2026-07-30

トランザクションの入れ子検出と、設計規約（セッション型の扱い・サービスの口の立て方・
DTO のネスト）のドキュメント整備。
**破壊的変更を含みます。**
...
- `docs/architecture.md` の記述を更新しました。「セッション型がドメイン層から見えることの
  是非とプロジェクト分割時の選択肢」という記述を、上記のセッション型の扱いの規約
  （型引数として受け取り、具体型はインフラ層と DI 登録でのみ確定させる）に合わせて
  書き換えています。
